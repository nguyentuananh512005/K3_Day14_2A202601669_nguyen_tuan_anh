...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"Answer: {answer}\n"
            f"Rubric: {rubric}\n"
            "Please score the answer against each rubric criterion as a float 0.0 to 1.0 in JSON."
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(scores_batch) >= 2:
            ...
            if first_item_scores and other_items_scores:
                first_avg = sum(first_item_scores) / len(first_item_scores)
                other_avg = sum(other_items_scores) / len(other_items_scores)
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list[EvalResult], baseline_results: list[EvalResult]) -> dict[str, Any]:
        ...
        new_f = (sum(r.faithfulness for r in new_results) / new_len) if new_len else 0.0
        new_r = (sum(r.relevance for r in new_results) / new_len) if new_len else 0.0
        new_c = (sum(r.completeness for r in new_results) / new_len) if new_len else 0.0

        base_len = len(baseline_results)
        base_f = (sum(r.faithfulness for r in baseline_results) / base_len) if base_len else 0.0
        base_r = (sum(r.relevance for r in baseline_results) / base_len) if base_len else 0.0
        base_c = (sum(r.completeness for r in baseline_results) / base_len) if base_len else 0.0
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        min_count = sum(1 for s in (f_score, r_score, c_score) if s == min_score)

        if min_count > 1:
            return "Multiple issues detected — review full pipeline"
        elif min_score == f_score:
    ...
    def generate_improvement_log(self, failures: list[EvalResult], suggestions: list[str]) -> str:
        ...
        for idx, f in enumerate(failures, start=1):
            ...
            suggested_fix = (
                ...
                else "Investigate failure cause and update prompt/retriever"
            )
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination", 0) > 0 or any(f.faithfulness < 0.5 for f in failures):
            ...
            suggestions.append("Refine system prompt with strict grounding instructions and refusal guidelines")

        if (
            categories.get("irrelevant", 0) > 0
            or categories.get("off_topic", 0) > 0
        ...
        defaults = [
            ...
            "Optimize retrieval re-ranking to prioritize highly relevant context chunks",
        ]
        for d in defaults:
